...
export const personalApi = {
  ...
  gotoSearch:(query:string,limit=30)=>call<GotoResult[]>("goto_search",{query,limit}), fullTextSearch:(query:string,limit=30)=>call<FullTextResult[]>("full_text_search",{query,limit}), dashboard:(profile_id:string)=>call<Dashboard>("dashboard_aggregate",{profileId:profile_id}), calendarOptions:(profile_id:string)=>call<CalendarOptions>("get_calendar_options",{profileId:profile_id}), saveCalendarOptions:(options:CalendarOptions)=>call<CalendarOptions>("set_calendar_options",{options}), dashboardPreferences:(profile_id:string)=>call<DashboardPreferences>("get_dashboard_preferences",{profileId:profile_id}), saveDashboardPreferences:(preferences:DashboardPreferences)=>call<DashboardPreferences>("set_dashboard_preferences",{preferences}),
};

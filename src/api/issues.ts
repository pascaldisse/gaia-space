...
export const planningApi = {
  ...
  issue: (id:string) => call<IssueDetail|null>("get_issue_detail", { id }),
  ...
  columns:(board_id:string)=>call<BoardColumn[]>("list_board_columns",{boardId:board_id}), saveColumn:(input:Omit<BoardColumn,"id"|"ordering">&{id?:string;ordering?:number})=>call<BoardColumn>("save_board_column",{input}), deleteColumn:(id:string)=>call<void>("delete_board_column",{id}),
  boardIssues:(board_id:string,sprint_id?:string)=>call<Issue[]>("list_board_issues",{boardId:board_id,sprintId:sprint_id}), backlog:(board_id:string)=>call<Issue[]>("list_backlog_issues",{boardId:board_id}), move:(board_id:string,issue_id:string,column_id:string,sprint_id?:string,position?:number)=>call<void>("move_issue_on_board",{boardId:board_id,issueId:issue_id,columnId:column_id,sprintId:sprint_id,swimlaneId:null,position}), remove:(board_id:string,issue_id:string)=>call<void>("remove_issue_from_board",{boardId:board_id,issueId:issue_id}),
  sprints:(board_id?:string)=>call<Sprint[]>("list_sprints",{boardId:board_id}), createSprint:(input:Omit<Sprint,"id"|"state"|"archived">&{id?:string})=>call<Sprint>("create_sprint",{input}), launchSprint:(id:string)=>call<void>("launch_sprint",{id}), closeSprint:(id:string)=>call<void>("close_sprint",{id}), deleteSprint:(id:string)=>call<void>("delete_sprint",{id}),

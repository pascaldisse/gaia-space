...
export type Account = { id: string; name: string; website: string; locationCount: number; employees: string; decisionMaker?: string; labels: string[]; software: string; source: string; owner: string; stage: CrmStage; status: "Aktiv" | "Verloren"; dealScope: "Betrieb" | "Standorte"; locations: Location[] };

const KEY = "gaia.crm.prototype.v1";
export const id = (prefix: string) => `${prefix}-${crypto.randomUUID()}`;
const seed = (): Account[] => [{
  id: "account-example", name: "Beispiel Optik GmbH", website: "", locationCount: 2, employees: "12", labels: ["Gründungskunde"], software: "", source: "Beispieldaten", owner: "Jannes", stage: "Qualified", status: "Aktiv", dealScope: "Betrieb", locations: [
...
export const loadCrm = (): Account[] => { try { const raw = localStorage.getItem(KEY); const accounts: Account[] = raw ? JSON.parse(raw) : seed(); const stageOf = (stage: string | undefined): CrmStage => stage === "Verhandlung" ? "Abgeschlossen" : (CRM_STAGES.includes(stage as CrmStage) ? stage as CrmStage : "Non-Qualified"); return accounts.map(account => ({ ...account, locationCount: account.locations.length, stage: stageOf(account.stage ?? account.locations[0]?.stage), status: account.status ?? account.locations[0]?.status ?? "Aktiv", labels: account.labels ?? [], dealScope: account.dealScope ?? "Betrieb", locations: account.locations.map(location => ({ ...location, stage: stageOf(location.stage), employees: location.employees ?? "" })) })); } catch { return seed(); } };

...
/** A deal is a sales opportunity. It links to a business; it is never the business itself. */
export type Deal = { id: string; title: string; stage: CrmStage; status: "Aktiv" | "Gewonnen" | "Verloren"; owner: string; source: string; value: string; expectedClose: string; primaryContactId?: string; labels: string[]; nextStep: string; nextStepDate: string; notes: Note[]; activities: Activity[]; createdAt: string }; 
...
export type Account = { id: string; name: string; website: string; locationCount: number; employees: string; decisionMaker?: string; labels: string[]; software: string; source: string; owner: string; stage: CrmStage; status: "Aktiv" | "Verloren"; dealScope: "Betrieb" | "Standorte"; locations: Location[]; deals: Deal[] };

const KEY = "gaia.crm.prototype.v1";
export const id = (prefix: string) => `${prefix}-${crypto.randomUUID()}`;
const seed = (): Account[] => [{
  id: "account-example", name: "Beispiel Optik GmbH", website: "", locationCount: 2, employees: "12", labels: ["Gründungskunde"], software: "", source: "Beispieldaten", owner: "Jannes", stage: "Qualified", status: "Aktiv", dealScope: "Betrieb", deals: [{ id: "deal-example", title: "Beispiel Optik GmbH", stage: "Qualified", status: "Aktiv", owner: "Jannes", source: "Beispieldaten", value: "", expectedClose: "", labels: ["Gründungskunde"], nextStep: "Erstgespräch terminieren", nextStepDate: "", notes: [], activities: [], createdAt: new Date().toISOString() }], locations: [
...
export const loadCrm = (): Account[] => { try { const raw = localStorage.getItem(KEY); const accounts: Account[] = raw ? JSON.parse(raw) : seed(); const stageOf = (stage: string | undefined): CrmStage => stage === "Verhandlung" ? "Abgeschlossen" : (CRM_STAGES.includes(stage as CrmStage) ? stage as CrmStage : "Non-Qualified"); return accounts.map(account => { const stage = stageOf(account.stage ?? account.locations[0]?.stage); const status = account.status ?? account.locations[0]?.status ?? "Aktiv"; const locations = account.locations.map(location => ({ ...location, stage: stageOf(location.stage), employees: location.employees ?? "" })); const deals = account.deals?.map(deal => ({ ...deal, stage: stageOf(deal.stage), labels: deal.labels ?? [], notes: deal.notes ?? [], activities: deal.activities ?? [] })) ?? [{ id: id("deal"), title: account.name, stage, status: status === "Verloren" ? "Verloren" : "Aktiv", owner: account.owner ?? "", source: account.source ?? "", value: "", expectedClose: "", labels: account.labels ?? [], nextStep: locations[0]?.nextStep ?? "", nextStepDate: locations[0]?.nextStepDate ?? "", notes: [], activities: [], createdAt: new Date().toISOString() }]; return { ...account, locationCount: locations.length, stage, status, labels: account.labels ?? [], dealScope: account.dealScope ?? "Betrieb", locations, deals }; }); } catch { return seed(); } };

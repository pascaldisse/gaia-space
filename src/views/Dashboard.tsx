...
export default function Dashboard() {
  ...
  return (
    <section class="dashboard-view">
          ...
          <section class="today-next">
            ...
            <div class="tn-grid">
              <section class="tn-col tn-tasks">
                <header><h3>Tasks to action <span>{data().open_todos.length}</span></h3><button class="tn-link" onClick={() => navigate("To-Do")}>Open</button></header>
              ...
              <section class="tn-col tn-risk">
                <header><h3>Project deadlines</h3><button class="tn-link" onClick={() => navigate("Projects")}>Open</button></header>
                <ul class="tn-risklist"><For each={future().filter((item) => item.kind === "deadline").slice(0, AGENDA_LIMIT)}>{(item) =>
                  <li><button onClick={() => navigate("Projects")}><i class="tn-riskdot high" /><span class="tn-risk-body"><strong>{item.title}</strong><small>{dayLabel(item.starts_at)}</small></span><b class="tn-riskbadge high">Due</b></button></li>
                }</For></ul>
              ...
              <section class="tn-col tn-agenda">
                <header><h3>Agenda</h3><button class="tn-link" onClick={() => navigate("Calendar")}>Open</button></header>
                <ul class="tn-agendalist"><For each={future().filter((item) => item.kind === "meeting").slice(0, AGENDA_LIMIT)}>{(item) =>
                  <li><span class="tn-when"><time>{dayLabel(item.starts_at)}</time><small class="tn-hour">{timeLabel(item.starts_at)}</small></span><span class="tn-meet-body"><strong>{item.title}</strong></span><button class="tn-meet-btn ghost" onClick={() => navigate("Calendar")}>Open</button></li>
                }</For></ul>
            ...
            <div class="co-body">
              ...
              <aside class="co-agenda">
                ...
                <ul><For each={selectedItems().slice(0, UPCOMING_LIMIT)}>{(item) =>
                  <li class={item.kind}><button onClick={() => navigate(item.kind === "deadline" ? "Projects" : item.kind === "task" ? "To-Do" : "Calendar")}><time class="co-time">{dayLabel(item.starts_at)}</time><strong>{item.title}</strong></button></li>
                }</For></ul>
      ...
      <Show when={profileId()}><button class="dashboard-refresh" onClick={refresh}>Refresh dashboard</button></Show>
    </section>
  );
}

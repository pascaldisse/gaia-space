...
export default function Calendar() {
  ...
  return <section class="calendar-view"><WorkspaceHeader icon="□" title={route().projectId ? "Project calendar" : "Calendar"} actions={<div class="calendar-controls"><button aria-label="Previous range" onClick={()=>shift(-1)}>←</button><strong>{cursor().toLocaleDateString(undefined,{month:"long",year:"numeric"})}</strong><button aria-label="Next range" onClick={()=>shift(1)}>→</button><button classList={{active:view()==="month"}} onClick={()=>setView("month")}>Month</button><button classList={{active:view()==="week"}} onClick={()=>setView("week")}>Week</button></div>}>Meetings, assigned task dates, and project deadlines visible to your session.</WorkspaceHeader><Show when={items.loading}><p>Loading calendar…</p></Show><Show when={items.error}><p class="calendar-error" role="alert">Calendar failed to load: {String(items.error)}</p></Show><div classList={{"calendar-grid":true,week:view()==="week"}}><For each={["Sun","Mon","Tue","Wed","Thu","Fri","Sat"]}>{day=><strong class="calendar-weekday">{day}</strong>}</For><For each={days()}>{day=><article classList={{"calendar-day":true,muted:view()==="month"&&day.getMonth()!==cursor().getMonth()}}><time>{day.getDate()}</time><For each={events(day)}>{event=><button class={`calendar-event ${event.kind}`} onClick={()=>setSelected(event)}><span>{labels[event.kind]}</span> {event.title}</button>}</For></article>}</For></div><Show when={selected()}>{event=><aside class="calendar-detail"><div><h2>{event().title}</h2><p>{labels[event().kind]} · {new Date(event().starts_at*1000).toLocaleString()}</p></div><button onClick={()=>setSelected(undefined)}>Close</button></aside>}</Show></section>;
}

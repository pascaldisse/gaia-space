...
export default function ProjectHome(props: { project?: Project }) {
  ...
  return <section class="ph-view project-home" aria-label={`${project()?.name ?? "Project"} dashboard`}>
    <PageHeader kicker={project()?.name ?? "Project unavailable"} title="Project overview" subline="What is running, who is on it, and when it is due — each figure opens the surface that owns it." actions={<button class="ghost small" onClick={() => void refetch()}>Refresh</button>} />

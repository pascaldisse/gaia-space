            ...
            <aside class="cw-rail" aria-label="Project details">
              ...
              <Show when={teamOpen()}>
                <section id="cw-project-team" class="cw-card">
                  <h2>Team</h2>
                  <Show when={(memberIds() ?? []).length} fallback={<EmptyState title="Nobody is in this project yet" actions={<GhostPill {...linkProps({ view: "Project Settings", projectId: value().id })}>Add people</GhostPill>} />}>
                    <For each={memberIds()}>

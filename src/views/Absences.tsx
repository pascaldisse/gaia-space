...
import { currentUser, humanError, isWeb, profileId as sessionProfile, profiles as sessionProfiles } from "../session";
...
export default function Absences() {
  ...
  const canApprove = () => !isWeb() || currentUser()?.role === "admin";
      ...
      <Show when={showForm()}>
        <form class="timeoff-form" onSubmit={save}>
          ...
          <div class="timeoff-form-foot">
            <Show when={canApprove()}><label class="fld-check"><input type="checkbox" checked={form().approved} onChange={(e) => setForm({ ...form(), approved: e.currentTarget.checked })} /> Already approved</label></Show>
        ...
        <section class="availability-board" aria-label="Availability at a glance">
          ...
          <div class="availability-panel pending">
            ...
            <Show when={pendingList().length} fallback={<p>Everything is approved.</p>}>
              <ul><For each={pendingList().slice(0, 4)}>{(a) => <li><div><strong>{personName(a.profile_id)}</strong><span>{a.reason_type} · {fmtRange(a.date_from, a.date_to)}</span></div><Show when={canApprove()}><button class="ghost small" onClick={() => update(a, { approved: true })}>Approve</button></Show></li>}</For></ul>
            </Show>
          </div>
        </section>
            ...
            return (
              <li class="timeoff-row" classList={{ [`is-${phase.key}`]: true, past: phase.key === "past" }}>
                ...
                <div class="timeoff-row-actions">
                  <Show when={canApprove()}><button class="ghost small" classList={{ active: a.approved }} onClick={() => update(a, { approved: !a.approved })}>
              ...
              <div class="rail-rows">
                <For each={pendingList().slice(0, 6)}>{(a) =>
                  <div class="rail-item">
                    ...
                    <Show when={canApprove()}><div class="rail-item-act"><button class="ghost small" onClick={() => update(a, { approved: true })}>Approve</button></div></Show>
                  </div>}</For>
              </div>
            </Show>
          </div>

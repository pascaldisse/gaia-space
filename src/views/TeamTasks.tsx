...
export default function TeamTasks() {
  ...
  const visible = createMemo(() => (todos() ?? []).filter(todo => {
  ...
  const groups = createMemo(() => {
    ...
    return [...byProject.entries()]
      .map(([projectId, items]) => ({ projectId, name: projectName(projectId), items }))
  ...
  return <section class="planning-view team-tasks-view">
    ...
    <Show when={todos.error}><p class="planning-error" role="alert">Could not load team tasks: {String(todos.error)}</p></Show>
    ...
    <Show when={todos.loading}><p class="hint">Loading team tasks…</p></Show>
    <Show when={!todos.loading && !groups().length}><p class="empty-state">No team tasks match these filters.</p></Show>

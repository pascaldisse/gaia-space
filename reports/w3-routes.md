...
| `project_member_ids` | `src-tauri/src/lib.rs:350` | no literal UI or test call in audit scope | remove registration |
...
| `list_project_member_ids` | UI `src/api/personal.ts:18`; tests `src/views/issues.detail.test.tsx:83`; desktop handler exposes `project_member_ids` at `src-tauri/src/lib.rs:349` | rename desktop command to UI contract; update Rust direct callers |

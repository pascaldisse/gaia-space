...
TOTALS (8/8 audited, 356 rows): done 59 · partial 180 · stub 4 · missing 113. Matrix NOT green — steering loop active.
...
Progress (feat/w7-devenv ☀Vishnu): dev environment lifecycle record (V55 `dev_environments`): per-environment idle timeout, activity touch, idle sweep hibernating with preserved home/working trees, resume that refuses a snapshot-less hibernation, standby ("hot pool") claim by conditional UPDATE, soft delete that drops preserved storage — plus the `Dev Environments` right group (all ten `Rd.*` codes with KB implication edges), enforced for create/manage/join. Commands are Tauri + `/api/cmd` `CommandPolicy::Session`; view `src/views/DevEnvironments.tsx`. Tests: `devenv::tests::*` (3) ⇒ idle-hibernation and hot-pool rows missing→partial. No provisioner: no VM/container is started, so KB §3.1 #1 stays missing. HTTP route VERIFIED: `space-server::tests::dev_environment_lifecycle_is_reachable_over_http` drives create→touch→idle sweep→resume over `/api/cmd` and asserts the unauthenticated call is rejected.
...
35 rows — done 11 · partial 16 · missing 8.

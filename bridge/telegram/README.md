...
Config → `TELEGRAM_BOT_TOKEN` required · `SPACE_PASSWORD` required · `SPACE_CHANNEL_ID` required · `SPACE_SERVER_URL` default `http://127.0.0.1:8090` · `SPACE_USERNAME` required; create this dedicated Space service account before starting · `TELEGRAM_POLL_INTERVAL_MS` default `1500` · `TELEGRAM_STATE_PATH` default `bridge/telegram/state.json`.

...
fn cipher() -> Result<ChaCha20Poly1305, String> {
    let raw = std::env::var(KEY_ENV)
        .map_err(|_| format!("{KEY_ENV} is not set: refusing to store a secret in plaintext"))?;
    let bytes =
        hex::decode(raw.trim()).map_err(|_| format!("{KEY_ENV} must be 64 hex characters"))?;
    if bytes.len() != 32 {
        return Err(format!(
...
mod tests {
    ...
    fn tampering_is_detected() {
        with_key(Some(&"33".repeat(32)), || {
            ...
            raw[last] ^= 0x01;
            assert!(
                open(&STANDARD.encode(raw)).is_err(),
    ...
    fn a_different_key_cannot_open_it() {
        let sealed = with_key(Some(&"44".repeat(32)), || {
            seal("refresh-token-value").unwrap()
        });
    ...
    fn without_a_key_we_refuse_rather_than_store_plaintext() {
        with_key(None, || {
            assert!(!configured());
            assert!(
                seal("refresh-token-value").is_err(),
                "no key must mean no storage, not plaintext"
            );
        });
    }
}

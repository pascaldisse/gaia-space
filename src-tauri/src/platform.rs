...
fn hydrate_location_equipment(c: &Connection, location: &mut Location) -> Result<()> {
    let mut statement = err(c.prepare(
        "SELECT equipment FROM location_equipment WHERE location_id=?1 ORDER BY equipment",
    ))?;
    let rows = statement
        .query_map([&location.id], |row| row.get(0))
        .map_err(|error| error.to_string())?;
    location.equipment = rows
...
pub fn list_locations() -> Result<Vec<Location>> {
    ...
    let rows = statement
        .query_map([], location_row)
        .map_err(|error| error.to_string())?;
    let mut locations = rows
        .collect::<std::result::Result<Vec<_>, _>>()
        .map_err(|error| error.to_string())?;
    for location in &mut locations {
        hydrate_location_equipment(&c, location)?;
    }
    Ok(locations)
}
#[cfg_attr(feature = "desktop", tauri::command)]
pub fn save_location(mut location: Location) -> Result<Location> {
    if location.id.trim().is_empty() {
        location.id = new_id("location");
    }
    if location.name.trim().is_empty() || location.timezone.trim().is_empty() {
        return Err("Location name and timezone are required".into());
    }
    let _: serde_json::Value = serde_json::from_str(&location.work_schedule_json)
    ...
    for equipment in &location.equipment {
        ...
        if !equipment.is_empty() {
            err(tx.execute(
                "INSERT INTO location_equipment(location_id,equipment) VALUES(?1,?2)",
                params![location.id, equipment],
            ))?;
        }
    }
    err(tx.commit())?;
...
pub fn location_channel(location_id: String) -> Result<Option<String>> {
    ...
    err(c
        .query_row(
            "SELECT channel_id FROM locations WHERE id=?1 AND archived=0",
            [location_id],
            |row| row.get(0),
        )
        .optional())

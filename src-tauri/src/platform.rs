...
const CF_TYPES: &[&str] = &[
    "text", "text_list", "int", "int_list", "enum", "enum_list", "open_enum", "open_enum_list", "bool", "date", "datetime", "percentage", "fraction", "profile", "profile_list", "team", "location", "project", "url", "contact", "contact_list", "autonumber", "issue", "issue_list",
];
...
fn validate_cf_shape(cf_type: &str, constraints_json: Option<&str>) -> Result<()> {
    ...
    if matches!(cf_type, "enum" | "enum_list") {
        let constraints: serde_json::Value = match constraints_json { Some(s) if !s.is_empty() => serde_json::from_str(s).map_err(|e| format!("Invalid constraints JSON: {e}"))?, _ => return Err("Enum custom fields require constraints.options".into()) };
        let options = constraints.get("options").and_then(|v| v.as_array()).filter(|a| !a.is_empty()).ok_or("Enum custom fields require a non-empty constraints.options array")?;
        if !options.iter().all(|o| o.is_string()) { return Err("constraints.options must be an array of strings".into()); }
    }
    Ok(())
}
...
fn validate_cf_value(
    ...
    let strings = |v: &serde_json::Value| -> Result<Vec<&str>> {
        if matches!(cf_type, "text_list" | "enum_list" | "open_enum_list" | "profile_list" | "contact_list" | "issue_list") { v.as_array().ok_or_else(|| "Expected an array".to_string())?.iter().map(|item| item.as_str().ok_or_else(|| "Expected an array of strings".to_string())).collect() } else { Ok(vec![v.as_str().ok_or("Expected a string value")?]) }
    };
    match cf_type {
        "bool" => if !value.is_boolean() { return Err("Expected a boolean value".into()); },
        "text" | "text_list" | "open_enum" | "open_enum_list" | "location" | "contact" | "contact_list" | "team" | "project" | "profile" | "profile_list" | "issue" | "issue_list" => for s in strings(&value)? { if let Some(min) = constraints.get("minLength").and_then(|v| v.as_u64()) { if (s.len() as u64) < min { return Err(format!("Value shorter than minLength {min}")); } } if let Some(max) = constraints.get("maxLength").and_then(|v| v.as_u64()) { if (s.len() as u64) > max { return Err(format!("Value longer than maxLength {max}")); } } },
        "url" => { let s = value.as_str().ok_or("Expected a URL string")?; if !(s.starts_with("http://") || s.starts_with("https://")) { return Err("Expected an http(s) URL".into()); } },
        "int" | "autonumber" => if value.as_i64().is_none() { return Err("Expected an integer value".into()); },
        "int_list" => if !value.as_array().map(|a| a.iter().all(|v| v.as_i64().is_some())).unwrap_or(false) { return Err("Expected an array of integers".into()); },
        "percentage" | "fraction" => { let n = value.as_f64().ok_or("Expected a number")?; if !(0.0..=1.0).contains(&n) { return Err("Expected a value from 0 to 1".into()); } },
        "date" => { let s = value.as_str().ok_or("Expected an ISO date string (YYYY-MM-DD)")?; chrono::NaiveDate::parse_from_str(s, "%Y-%m-%d").map_err(|_| format!("Invalid date '{s}', expected YYYY-MM-DD"))?; },
        "datetime" => { let s = value.as_str().ok_or("Expected an ISO datetime string")?; chrono::DateTime::parse_from_rfc3339(s).map_err(|_| format!("Invalid datetime '{s}', expected RFC3339"))?; },
        "enum" | "enum_list" => { let options = constraints.get("options").and_then(|v| v.as_array()).ok_or("Enum field is missing constraints.options")?; for s in strings(&value)? { if !options.iter().any(|o| o.as_str() == Some(s)) { return Err(format!("'{s}' is not one of the allowed enum options")); } } },

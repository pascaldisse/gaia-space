...
pub fn goto_search_scoped(query:String,limit:Option<i64>,profile_id:String,allow_all:bool)->Result<Vec<GotoResult>>{let term=query.trim();if term.is_empty(){return Ok(Vec::new());}let pattern=format!("%{}%",term.to_lowercase());let exact=term.to_lowercase();let c=db::conn()?;let mut s=err(c.prepare("SELECT id,entity_type,title,details,score FROM (
SELECT id,'profile' entity_type,display_name title,username details,CASE WHEN lower(display_name)=?2 THEN 100 ELSE 50 END score FROM profiles WHERE lower(display_name) LIKE ?1 OR lower(username) LIKE ?1
...
) ORDER BY score DESC,title COLLATE NOCASE LIMIT ?5"))?;let rows=err(s.query_map(params![pattern,exact,profile_id,allow_all,limit.unwrap_or(30).clamp(1,100)],|r|Ok(GotoResult{id:r.get(0)?,entity_type:r.get(1)?,title:r.get(2)?,details:r.get(3)?,score:r.get(4)?})))?.collect::<std::result::Result<Vec<_>,_>>().map_err(|e|e.to_string());rows}

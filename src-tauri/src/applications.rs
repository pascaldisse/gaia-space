...
pub(crate) fn enqueue_webhook_delivery(
    ...
    let exists: bool = c
        .query_row(
            "SELECT EXISTS(SELECT 1 FROM webhook_subscriptions WHERE id=?1 AND enabled=1)",
            [webhook_id],
            |r| r.get(0),
        )
        .map_err(|e| e.to_string())?;
    if !exists {
        return Err("Webhook subscription not found or disabled".into());
    };
    c.execute("INSERT INTO webhook_deliveries(id,webhook_id,payload_json,status,next_attempt_at) VALUES(?1,?2,?3,'PENDING',unixepoch())",params![new_delivery_id()?,webhook_id,serde_json::to_string(payload).map_err(|e|e.to_string())?]).map_err(|e|e.to_string())?;
    Ok(())
}
#[cfg_attr(feature = "desktop", tauri::command)]
